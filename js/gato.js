...
//SIMPLEMENTE COMPRUEBA UN TABLERO NORMAL DE TAC TOE, DEVUELVE 1 o -1 si un jugador específico ha ganado, devuelve 0 si nadie ha ganado.
                ...
                for(var trr = 0; trr < 9; trr++){
                    ...
                    if(checkWinCondition(position[mm]) === 0){
                        if(position[mm][trr] === 0){
                            position[mm][trr] = ai;
                            //tmpPlay = pickBoard(posicion, verdadero);
...
//Función inútil, solo aquí para poder estudiar minimax
...
//También una función que ya no se usa, mis primeros intentos de hacer que la IA elija un tablero para jugar cuando puede jugar en cualquier lugar
...
function game(){
    ...
    if(currentTurn === -1 && gameRunning && AIACTIVE){
        ...
        RUNS = 0; //Solo una variable donde almaceno cuántas veces se ha ejecutado minimax
        ...
        if(currentBoard === -1 || checkWinCondition(boards[currentBoard]) !== 0){
            ...
            if(MOVES < 10) {
                savedMm = miniMax(boards, -1, Math.min(4, count), -Infinity, Infinity, true); //Poner math.min asegura que minimax no se ejecute cuando el tablero esté lleno
            }else if(MOVES < 18){
        ...
        //Simplemente hace un movimiento predeterminado rápido por si todo lo demás falla
        for (var i = 0; i < 9; i++) {
        ...
        if(bestMove !== -1) { //Esta condición solo debería ser falsa si el tablero está lleno, pero está aquí por si acaso
            ...
            //Y aquí en realidad ejecutamos minimax y agregamos esos valores a la matriz
            for(var b = 0; b < 9; b++){
                if(checkWinCondition(boards[currentBoard]) === 0){
                    if (boards[currentBoard][b] === 0) {
                        boards[currentBoard][b] = ai;
                        var savedMm;
                        //Observe el apilamiento, al comienzo del juego, la profundidad es mucho menor que al final
                        if(MOVES < 20){
            ...
            //En realidad coloca la cruz/cero
    ...
    //Dibuja los PEQUEÑOS ceros y cruces
    ctx.lineWidth = 5;
    ...
    if(gameRunning){
        ...
        //Una vez más, cuente la cantidad de casillas jugables, si es 0, el juego está empatado.
        var countw = 0;
        for(var bt = 0; bt < boards.length; bt++){
            if(checkWinCondition(boards[bt]) === 0){
    ...
    //Dibuja los GRANDES ceros y cruces
    ...
    //TABLERO DE DESTACADOS PARA JUGAR
